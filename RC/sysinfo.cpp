...
void getCPU(SystemInfo *localMachine,
	...
	while (pEnumerator) {
		...
		if (processor.find(L"@", 0) == string::npos) {
			...
			_swprintf(maxClockBuff, L"%.1lf", maxClockInGhZ);
...
void getRAM(SystemInfo *localMachine,
	...
	while (pEnumerator) {
		...
		//wchar_t capacityStrBuff[10];
		wchar_t clockStrBuff[10];
		...
		_swprintf(clockStrBuff, L"%d", clock);
...
void getGPU(SystemInfo *localMachine,
	...
	while (pEnumerator) {
		...
		_swprintf(vRamCharBuff, L"%.0lf MB", vRAMmegaBytes);
...
void getMonitor(SystemInfo *localMachine,
	...
	while (pEnumerator) {
		...
		_swprintf(resAndFreqBuff, L"(%dx%d@%dHz)", dimensionsAndFrequency[0],
...
void getStorage(SystemInfo *localMachine,
	...
	while (pEnumerator) {
		...
		capacityGiBDbl = (UINT64)(capacityBytes / pow(1024, 3));
...
void getUptime(SystemInfo *localMachine) {
	...
	UINT64 uptimeDays = 0;
	wchar_t *format = NULL;
	wchar_t formattedTimeString[256] = { 0 };
	...
	if (uptimeDays != 0) {
		if (uptimeHours != 0) {
			if (uptimeHours > 1) {
				_swprintf(formattedTimeString, uptimeDays > 1 ? L"%llu days and %llu hours" : L"%llu day and %llu hours", uptimeDays, uptimeHours);
			}
			else {
				_swprintf(formattedTimeString, uptimeDays > 1 ? L"%llu days and %llu hour" : L"%llu day and %llu hour", uptimeDays, uptimeHours);

			}
			_swprintf(formattedTimeString, L"%llu days and %llu hours", uptimeDays, uptimeHours);
		}
		else {
			_swprintf(formattedTimeString, uptimeDays > 1 ? L"%llu days" : L"%llu day", uptimeDays, uptimeHours);
		}
	}
	else {
		if (uptimeHours < 1) {
			_swprintf(formattedTimeString, L"%s", L"Less than an hour");
		}
		else if (uptimeHours == 1) {
			_swprintf(formattedTimeString, L"%llu hour", uptimeHours);
		}
		else {
			_swprintf(formattedTimeString, L"%llu hours", uptimeHours);
		}
	}
